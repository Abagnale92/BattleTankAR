...
public class Generazione3 : Photon.MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (!PhotonNetwork.isMasterClient && GameObject.Find("ImageTargetCity").GetComponent<DefaultTrackableEventHandler>().find) {
            photonView.RPC("TerrenoTrovatoC2", PhotonTargets.All);
        }
        if(PhotonNetwork.isMasterClient &&  GameObject.Find("ImageTargetCity").GetComponent<DefaultTrackableEventHandler>().find){
            photonView.RPC("TerrenoTrovatoC", PhotonTargets.All);
        }
        ...
        if (!gm.gameHasEnded)
        {
            if (random == n)
            {
                ...
                {
                    photonView.RPC("InstantiateBersaglioC", PhotonTargets.MasterClient);
                }
            }
   ...
   void InstantiateBersaglioC()
    ...
    [PunRPC]
    void TerrenoTrovatoC2() {
        find2 = true;
    }
    [PunRPC]
    void TerrenoTrovatoC()
    {
        find = true;
    }
